...
// src/classes/AppRouter.ts
var import_express = __toESM(require_express2(), 1);

class AppRouter {
  static instance = import_express.Router();
  static get router() {
    return AppRouter.instance;
  }
}
// src/types/enums/ControllerMethodsParams.ts
var ControllerMethodsParams;
((ControllerMethodsParams2) => {
  ControllerMethodsParams2["Path"] = "path";
  ControllerMethodsParams2["Method"] = "method";
  ControllerMethodsParams2["Middleware"] = "middleware";
  ControllerMethodsParams2["Router"] = "router";
})(ControllerMethodsParams ||= {});
// src/types/enums/HttpMethods.ts
var HttpMethods;
((HttpMethods2) => {
...
// src/types/enums/ServerConfigsParams.ts
var ServerConfigsParams;
((ServerConfigsParams2) => {
  ServerConfigsParams2["Router"] = "router";
  ServerConfigsParams2["GlobalMiddleware"] = "globalMiddleware";
})(ServerConfigsParams ||= {});
...
class Server {
  ...
  setupControllers() {
    ...
    this.controllers.forEach((controller) => {
      const router = Reflect.getOwnMetadata("router" /* Router */, controller);
      const globalMiddleware = Reflect.getOwnMetadata("globalMiddleware" /* GlobalMiddleware */, controller);
      if (globalMiddleware) {
...
function createHttpMethod(method) {
  return function(path) {
    return function(target, propertyKey) {
      Reflect.defineMetadata(propertyKey, method, target, "method" /* Method */);
      Reflect.defineMetadata(propertyKey, path, target, "path" /* Path */);
    };
  };
}
var Get = createHttpMethod("get" /* Get */);
var Post = createHttpMethod("post" /* Post */);
var Put = createHttpMethod("put" /* Put */);
var Patch = createHttpMethod("patch" /* Patch */);
var Delete = createHttpMethod("delete" /* Delete */);
...
function Controller(path) {
  ...
  return function(target) {
    ...
    _actionsNames.forEach((_actionName) => {
      const _method = Reflect.getOwnMetadata(_actionName, prototype.value, "method" /* Method */) ?? "";
      const _middlewares = Reflect.getOwnMetadata(_actionName, prototype.value, "middleware" /* Middleware */) ?? [];
      const _path = Reflect.getOwnMetadata(_actionName, prototype.value, "path" /* Path */) ?? "";
      router[_method](`${path}${_path}`, _middlewares, prototype.value[_actionName]);
    });
    Reflect.defineMetadata("router" /* Router */, router, target);
  };
}
// src/decorators/middleware.decorator.ts
function Middleware(handlers) {
  return function(target, propertyKey) {
    Reflect.defineMetadata(propertyKey, handlers, target, "middleware" /* Middleware */);
  };
}
// src/decorators/global-middleware.decorator.ts
function GlobalMiddleware(handlers) {
  return function(target) {
    Reflect.defineMetadata("globalMiddleware" /* GlobalMiddleware */, handlers, target);
  };
}
export {
  ServerConfigsParams,
